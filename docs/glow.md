...
|---|---|---|
| Linux / GNOME 46+ | Shell extension, triggered over D-Bus | shipped (extension extracted and pre-enabled via GSettings by `install --glow`; one logout/login later it is active - no manual enable step); `push-it install --glow` refuses clearly on non-GNOME sessions |
| macOS | helper app (Cocoa / Core Animation), universal binary | shipped in release binaries and via `install.sh`; built with `-tags glowhelper`; a plain `go install`/`go build` (no build tag) prints "built without -tags glowhelper" instead of drawing anything |
