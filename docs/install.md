...
`install.sh` downloads the latest release for your OS/arch (Linux and macOS, amd64 and arm64), verifies its `checksums.txt` entry (guards against a corrupted or truncated download; the checksums file ships with the same release, and there is no signature yet), puts `push-it` in `~/.local/bin`, and runs `push-it install "$@"` with whatever flags you passed. Env overrides: `PUSH_IT_VERSION` (default `latest`, or pin a tag like `v0.1.0`), `PUSH_IT_BIN_DIR` (default `~/.local/bin`). The one-liner re-attaches your terminal for the sound/Hue/glow questions even though it's piped through `sh`; on a box with no `/dev/tty` (a container, a CI runner), skip the questions by passing flags and `--yes` (`--sound --yes`, `--all --yes`, etc.) - bare `--all` without `--yes` will prompt for the Hue bridge and, with no terminal to answer from, save an enabled Hue component with an empty bridge and key.
