...
Celebrate every `git push`: a random "push it" clip, a rainbow burst on a Philips Hue light, and an animated rainbow frame around your screen for exactly as long as the clip plays. Each effect is optional. One binary, no runtime dependencies beyond your desktop's audio server (PulseAudio/PipeWire on Linux).
