...
- Linux playback no longer returns early on sub-second clips: pulse Drain() is a no-op once the reader hit EndOfData, so play now also waits out the clip's computed duration (manual verification with a 0.3 s WAV recorded at release).
...
- Hook log no longer double-prefixes glow/hue errors ("glow: glow: ..."); a cancelled glow reports the cancellation instead of "extension did not answer"; the Windows glow calls SetProcessDPIAware before measuring the screen so the band is crisp and correctly sized at >100% scaling.
